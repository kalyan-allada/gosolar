...
def read_data():
    # Data contains entries from year >= 2013
    data = pd.read_csv("data/openpv_ma_cost_final.csv", dtype={'zipcode': 'str'})
    return data
...
def cross_validate(data):
    ...
    #"installer" has been encoded with frequency at which they appear in the data set
    features_to_use = ["size_kw", "installer", "tech_1", "tracking_type", "Lat", "Long"]
    ...
    regressor = RandomForestRegressor(n_estimators = 150, min_samples_leaf= 5, \
                            max_depth=50)
...
def dump_model(regressor):   
    with open('data/cost_ma_final.pk','wb') as f:
...
def plot(regressor, y_train, y_valid, y_test, y_pred):
    ...
    sns.regplot(np.array(y_valid), residuals_train, color='g', marker='o', scatter_kws={'alpha':0.3})
    sns.regplot(np.array(y_pred), residuals_test, color='r', marker='o', scatter_kws={'alpha':0.3})

    plt.title("Residuals for training(green) and test(red) data", fontsize=40)
    plt.ylabel("Residuals",fontsize=40)
    plt.xlabel("Cost (USD)",fontsize=40)
    plt.tick_params(labelsize=30)
    plt.xlim(0,1.5e5)
    #fig1.savefig('./plots/residuals_lr.png')
    
    features = ["Array size", "Company", "Array type", "Tracking type", "Latitude", "Longitude"]
    ...
    sns.regplot(np.array(y_test), np.array(y_pred) , color='r', marker='o', scatter_kws={'alpha':0.3})
    plt.title("Random Forest", fontsize=40)
    plt.ylabel("Predicted cost (USD)",fontsize=40)
    plt.xlabel("Real cost (USD)",fontsize=40)
    plt.xlim(-500,1e5)
    plt.ylim(-500,1e5)
    plt.tick_params(labelsize=30)        
    return
